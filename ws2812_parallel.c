...
// we store value (8 bits + fractional bits of a single color (R/G/B/W) value)
// for multiple strips of pixels, in bit planes. bit plane N has the Nth bit of
// each strip of pixels.
typedef struct {
  // stored MSB first
  ...
  for (uint v = 0; v < value_length; v++) {
    ...
    for (uint i = 0; i < num_strips; i++) {
      if (v < strips[i]->data_len) {
        // todo clamp?
        uint32_t value =
            (strips[i]->data[v] * strips[i]->frac_brightness) >> 8u;
...
// double buffer the state of the pixel strip, since we update next version in
// parallel with DMAing out old version
...
// chain channel for configuring main dma channel to output from disjoint 8 word
// fragments of memory
...
void dma_init(PIO pio, uint sm) {
  ...
  dma_channel_configure(
      DMA_CB_CHANNEL, &chain_config,
      &dma_channel_hw_addr(DMA_CHANNEL)
           ->al3_read_addr_trig, // ch DMA config (target "ring" buffer size 4)
                                 // - this is (read_addr trigger)
      NULL,                      // set later
      1, false);
...
int main() {
  ...
  bool success = pio_claim_free_sm_and_add_program_for_gpio_range(
      &ws2812_parallel_program, &pio, &sm, &offset, WS2812_PIN_BASE,
      count_of(strips), true);
  hard_assert(success);

  ws2812_parallel_program_init(pio, sm, offset, WS2812_PIN_BASE,
                               count_of(strips), 800000);
  ...
  while (1) {
    ...
    for (int i = 0; i < 1000; ++i) {
      ...
      transform_strips(strips, count_of(strips), colors, NUM_PIXELS * 4,
                       brightness);
      dither_values(colors, states[current], states[current ^ 1],
                    NUM_PIXELS * 4);
